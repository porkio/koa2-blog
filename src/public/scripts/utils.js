...
>>>>>>> f1e1c1aa31a4e20ccaf280627ac47db85f117334
...
function randomStrGenerator(length) {
	...
	while (length > 0) {
		...
		const seed =
			parseInt(Math.random() * 43) +
			new Number(timestamp[parseInt(Math.random() * 12)])
		str += temp[seed]
		length--
	}
	return str
...
<<<<<<< HEAD
    if (typeof icon !== 'undefined' && iconsMap.get(icon)) {
...
function printLog(str) {
	console.log(
		'%c ' + str,
		'color: white; font-size: 20px; border-radius: 4px; text-shadow: 0px 2px 1px rgba(41, 41, 41, .6);'
	)
	const date = '2020/12/12'
	const a =
		'background: #606060; padding: 4px; color: #fff; border-radius: 2px 0 0 2px;'
	const b =
		'background: #1475B2; padding: 4px; color: #fff; border-radius: 0 2px 2px 0;'
		...
		img.addEventListener(
			...
			(e) => {
				...
				image.onload = function () {
					imgInfo.innerHTML = `<p style="color: #fff;">${
						img.title ? img.title : ''
					} &nbsp;&nbsp; Size: ${image.width} * ${image.height}</p>`
				}
