...
const databaseInit = async () => {
	const siteConfig = await SiteConfig.create({
		siteTitle: `猪肉王子的网络日志`,
		subTitle: '平平无奇的工作，为国家工作50年',
		views: 20208,
		copyRight: '© 2020 - 2022 猪肉王子',
	})
	...
	const article = await Article.create({
		...
		content:
			'在JavaScript语言中，bind、call、apply都是用来改变函数的this对象的指向的。说白了就是都可以用来代替另一个对象调用一个方法，将一个函数的对象的执行期上下文从初始的上下文改变为由当前对象的上下文。在深入讲解它们之前，我先讲一个小故事： 从前有一只猪和一只狗，突然有一天猪开口说话了：我是猪，我能吃又能睡！ 狗一听乐了，谁还不会吃和睡了？当即不服叫道：我是狗，我也能吃能睡！（我还会叫呢，但是这句它没有说出口，但是它的确有这个行为的能力）',
